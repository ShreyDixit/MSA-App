...
class TestPrepareData:
    ...
    @pytest.mark.parametrize("y_column_type", ["NIHSS Score", "Performance"])
    def test_valid_file_path_with_voxels(self, y_column_type):

        # Act
        X, y, voxels = prepare_data(
            self.data_file_path, self.y_column, y_column_type, self.voxels_file_path
        )
    ...
    @pytest.mark.parametrize("y_column_type", ["NIHSS Score", "Performance"])
    def test_valid_csv_file_path_without_voxels(self, y_column_type):
        # Arrange
        voxels_file_path = ""

        # Act
        X, y, voxels = prepare_data(
            self.data_file_path, self.y_column, y_column_type, voxels_file_path
        )
    ...
    def test_nihss_score_calculations(self):
        # Act
        X, y, voxels = prepare_data(
            self.data_file_path, self.y_column, "NIHSS Score", self.voxels_file_path
        )
        y_real = pd.read_excel(self.data_file_path)[self.y_column]
        assert y.min() == 0
